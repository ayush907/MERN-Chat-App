...
// function for creating a new group chat
export const newGroupChat = TryCatch(async (req, res, next) => {
    ...
    // if (members.length < 2) {
    //     return next(
    //         new ErrorHandler("group chat must have 3 members", 400)
    //     )
    // }
...
// function for getting the chats of the logged in user
...
// function for getting the groups of logged in user (matlab jo group user ne banaaya hai)
...
// function for adding the members in the group
...
// function for removing the members from the group 
...
export const leaveGroup = TryCatch(async (req, res, next) => {
    ...
    emitEvent(
        ...
        ALERT,
        chat.members,
        {message: `${user.name} has left ${chat.name} group`, chatId: chatId}
    )
...
// function for sending the attachments
export const sendAttachMents = TryCatch(async (req, res, next) => {
    ...
    // upload files here (clouinary mai upload karenge)
...
// function for getting the chat details
export const getChatDetails = TryCatch(async (req, res, next) => {

    if (req.query.populate === "true") {
        const chat = await Chat.findById(req.params.id).populate("members", "name avatar").lean()  //lean() ka use ksrke mongodb object normal javascript ka object bna jaata hai. 
